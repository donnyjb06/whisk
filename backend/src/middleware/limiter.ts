...
export const limiter: RateLimitRequestHandler = rateLimit({
  ...
  handler: (req: Request, res: Response, next: NextFunction) => {
    return next(new TooManyRequestsError("Rate limit exceeded. Please wait at least 15 minutes."))
  }
})
...
export const recipeGenerationLimiter: RateLimitRequestHandler = rateLimit({
  ...
  standardHeaders: true,
  handler: (req: Request, res: Response, next: NextFunction) => {
    return next(new TooManyRequestsError("Recipe generation limit exceeded. Please wait a minute."))
  }
})
