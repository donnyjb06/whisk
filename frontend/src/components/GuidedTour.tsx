...
const TourStepComponent: React.FC<{
	...
	children: ReactNode;
  onStepEnter?: () => void;
}> = ({ children, id, title, content, order, position, onStepEnter }) => {
	const { registerStep, unregisterStep, isActive, currentStepId } = useTour();
	const elementRef = useRef<HTMLDivElement>(null);

	const stepConfig = useMemo(
		() => ({ id, title, content, order, position, onStepEnter }),
		[id, title, content, order, position, onStepEnter]
	);
	...
	return (
		<div
			...
			className={isCurrentStep ? "relative z-10002 grow" : "relative grow"}
		>
			{children}
		</div>
	);
